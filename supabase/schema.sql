...
create table if not exists public.consultant_month_metrics (
  ...
  qa_calls_marked numeric not null default 0,
  qa_average numeric not null default 0,
  qa_complete boolean not null default false,
  qa_passed boolean not null default false,
  qa_failed boolean not null default false,
  compliance_payable_rate numeric not null default 1,
  current_fail_streak numeric not null default 0,
  commission_ineligible boolean not null default false,
  compliance_adjusted_mtd_commission numeric not null default 0,
  compliance_adjusted_projected_commission numeric not null default 0,
...
insert into public.contact_types (type_key, display_name, points_per_sale, expected_conversion_rate, sort_order, is_callback)
values
  ('outbound', 'Outbound', 1.0, 0.28, 1, false),
  ('inbound', 'Inbound', 0.7, 0.60, 2, false),
  ('schedule_a_call', 'Schedule a call', 0.7, 0.70, 3, false),
  ('cli', 'CLI', 1.5, 0.10, 4, false),
  ('crossvert', 'Crossvert', 1.8, 0.10, 5, false),
  ('crossvert_cli', 'Crossvert CLI', 1.8, 0.10, 6, false),
  ('billy', 'Billy', 1.5, 0.10, 7, false),
  ('billy_cli', 'Billy CLI', 1.5, 0.10, 8, false),
